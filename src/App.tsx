...
export default function Home() {
  ...
  useEffect(() => {
    const readShared = async () => {
      const token = location.hash.startsWith("#tamm=") ? location.hash.slice(6) : "";
      if (!token) return;
      try {
        const [keyPart, ivPart, cipherPart] = token.split(".");
        ...
        const parsed = JSON.parse(new TextDecoder().decode(plain)) as Agreement;
  ...
  const makeLink = async () => {
    ...
    const encrypted = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, new TextEncoder().encode(JSON.stringify(agreement)));
    const link = `${location.origin}${location.pathname}#tamm=${b64(rawKey)}.${b64(iv)}.${b64(new Uint8Array(encrypted))}`;
    setSharedLink(link);
