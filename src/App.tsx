...
export default function Home() {
  ...
  const makeLink = async (makeQr = false) => {
    ...
    if (makeQr) {
      ...
      const image = await QRCode.toDataURL(link, { width: 420, margin: 2, errorCorrectionLevel: "M", color: { dark: "#12382a", light: "#fffdf8" } });
      setQrDataUrl(image);
      setNotice(ar ? "تم إنشاء QR المشفّر محلياً." : "Encrypted QR created locally.");
    } else {
  ...
  const downloadQr = () => {
    ...
    anchor.download = `TAMM-QR-${Date.now()}.png`;
  ...
  const clearAll = () => {
    if (audioUrl) URL.revokeObjectURL(audioUrl);
    setChat(""); setAgreement(emptyAgreement); setSharedLink(""); setQrDataUrl(""); setReceipt(null); setApprover(""); setAudioUrl(""); setStage("compose");
    ...
    <main className="site" dir={ar ? "rtl" : "ltr"}>
      ...
      {(stage === "review" || stage === "shared") && <section className={`review ${stage === "shared" ? "shared-view" : ""}`} id="review">
        ...
        {stage === "review" && <div className="share-panel">
          ...
          <div className="share-actions"><button className="secondary" onClick={() => void downloadPdf()}>{ar ? "تنزيل PDF" : "Download PDF"}</button><button className="secondary" onClick={exportTamm}>{ar ? "حفظ ملف .tamm" : "Save .tamm file"}</button><button className="secondary" onClick={() => void makeLink(true)}>{ar ? "إنشاء QR خاص" : "Create private QR"}</button><button className="primary" onClick={() => void makeLink()}>{ar ? "نسخ الرابط المشفّر" : "Copy encrypted link"}</button></div>
          ...
          {qrDataUrl && <div className="qr-card"><img src={qrDataUrl} alt={ar ? "رمز QR مشفّر لاتفاق TAMM" : "Encrypted QR code for the TAMM agreement"} /><div><h4>{ar ? "امسح الرمز لفتح الاتفاق" : "Scan to open the agreement"}</h4><p>{ar ? "تم إنشاؤه على جهازك. لا يوجد حفظ أو رفع إلى خادم." : "Created on your device. Nothing is stored or uploaded to a server."}</p><button className="secondary" onClick={downloadQr}>{ar ? "تنزيل QR كصورة" : "Download QR image"}</button></div></div>}
        </div>}
